...
# def score_stone_type_match(row_stone, target_stone):
#     """Score stone type matching."""
#     if str(row_stone).upper() == target_stone.upper():
#         return SCORING_CONFIG["stone_match_exact"]
#     elif target_stone.upper() in str(row_stone).upper():
#         return SCORING_CONFIG["stone_match_partial"]
#     else:
#         return SCORING_CONFIG["stone_match_none"]

# def score_processing_match(row_processing, target_processing):
#     """Score processing type matching."""
#     if str(row_processing).upper() == target_processing.upper():
#         return SCORING_CONFIG["processing_match_exact"]
#     elif target_processing.upper() in str(row_processing).upper():
#         return SCORING_CONFIG["processing_match_partial"]
#     else:
#         return SCORING_CONFIG["processing_match_none"]

# def score_height_match(row_height, target_height):
#     """Score height matching."""
#     height_diff = abs(float(row_height) - target_height) if pd.notna(row_height) else 100
    
#     if height_diff < 0.1:
#         return SCORING_CONFIG["height_match_perfect"]
#     elif height_diff <= 1.0:
#         return SCORING_CONFIG["height_match_1cm"]
#     elif height_diff <= 2.0:
#         return SCORING_CONFIG["height_match_2cm"]
#     elif height_diff <= 5.0:
#         return SCORING_CONFIG["height_match_5cm"]
#     else:
#         return SCORING_CONFIG["height_match_default"]

# def calculate_priority_score(df, stone_type, processing_type, height):
#     """Calculate priority scores for filtered products."""
#     def score_row(row):
#         score = 0
#         score += score_stone_type_match(row['loai_da'], stone_type)
#         score += score_processing_match(row['gia_cong'], processing_type)
#         score += score_height_match(row['H'], height)
#         return score
